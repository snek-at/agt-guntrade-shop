...
const transformWidth = (width?: string) => (width ? 'scaleX(1.' + width.split("%")[0] + ')' : 'scaleX(1.3)')
const borderlinWidth = (width?: string) => (width ? width : '30%')

export const cardStyle = (borderline?: boolean, bwidth?: string, color?: string, left?: boolean) => css`
  .pcard {
    display: block;
    position: relative;
    ...
    * {
    ...
    &:hover .borderline {
      position: absolute;
      top: 0;
      animation: scale-up-hor-left 200ms cubic-bezier(0.680, -0.550, 0.265, 1.550) both;
