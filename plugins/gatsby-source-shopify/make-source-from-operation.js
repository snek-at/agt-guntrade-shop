...
function makeSourceFromOperation(finishLastOperation, completedOperation, cancelOperationInProgress, gatsbyApi, pluginOptions) {
    ...
    isPriorityBuild = process.env.GATSBY_IS_PR_BUILD !== `true` &&
        ...
        try {
            ...
            const results = await (0, node_fetch_1.default)(resp.node.url);
            ...
            await Promise.all(op
                .process(objects, (0, node_builder_1.nodeBuilder)(gatsbyApi, pluginOptions), gatsbyApi, pluginOptions)
                .map(async (promise) => {
            ...
            if (e instanceof errors_1.OperationError) {
                ...
                reporter.panic({
                    ...
                    context: {
                        sourceMessage: (0, shift_left_1.shiftLeft) `
